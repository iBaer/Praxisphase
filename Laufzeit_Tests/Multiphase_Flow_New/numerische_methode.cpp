		...
		for(int y = 0 ; y < CELLS[1]+2*ordnung+1 ; y++)
		  {
		    ...
		    for(int n = 0 ; n < n_eqns ; n++)
		      {
			if(smax1 < fabs(real(n))) smax1 = fabs(real(n));
			if(smax2 < fabs(real_b(n))) smax2 = fabs(real_b(n));
		      }
		  }
	      }
...
void numerische_methode::write()
{
  ...
  if (dimension==1) {
    added = to_string(CELLS[0])+"_"+name+"_"
      +to_string(dimension)+"d_"+to_string(variante)
      +".variant_"+"div"+to_string(teiler)
...
void numerische_methode::matrix_1d(double * values, int n, double uone, double utwo,
				   ...
				   double ccl, double g, double ct, double cref, int variante)
{
  switch(variante)
    ...
    case(2):
      ...
      values[1]=1;
      values[2]=0;
      values[3]=-(utwo*utwo)/(uone*uone)+ccl*(1-ccl)*uthree*uthree+cref*g*pow(done,g-1.0)*
	(ccl*done*done)/pow(done+(ccl-1.0)*uone,2.0);
      values[4]=(2*utwo)/uone;
      values[5]=uone*ccl*(1.0-ccl)*2.0*uthree;
      values[6]=-utwo/(uone*uone)*uthree + cref*(g*pow(dtwo,g-2.0)-(g*pow(dtwo,g-1.0))/done)*
	(ccl*done*done)/pow(done+(ccl-1.0)*uone,2.0);
      values[7]=uthree/uone;
      values[8]=(utwo/uone)+(1.0-2.0*ccl)*uthree;
      break;
 ...
 *****************************************************************************************/
void numerische_methode::matrix_2d(double * values_x, double * values_y, int n, double uone, double utwo,
