 ...
 */
vector< vector< vector< vector <double> > > > FORCE::calc_method_flux(int dir)
{
    ...
    case(1):
	    ...
	    cout<<"neqs "<<gs.neqs<<" gs.u.size() " << gs.u.size() << endl;
		  ...
		  0.5*(dt/dx)*(fd[0][i][0]-fd[0][i+1][0]);
                ...
                u_rie.zelle[i].p = ct*pow(u_rie.zelle[i].d,g);
            }
	    ...
	    int pos;
		  ...
		  pos = x+y*width;
		      ...
		      +(dt/2*dx)*(fd[4][x][y]-fd[4][x+1][y]);
                    u_rie_f.zelle[pos].p = ct*pow(u_rie_f.zelle[pos].d,g);
		    ...
		    u_rie_g.zelle[pos].uxr = 0.5*(cs[3][x][y]+cs[3][x][y+1])
		      ...
		      +(dt/2*dy)*(gd[4][x][y]-gd[4][x][y+1]);
                    u_rie_g.zelle[pos].p = ct*pow(u_rie_g.zelle[pos].d,g);
                }
            }
